...
export function handleCors(request: Request, env: Env): Response {
	const allowedOrigins = [request.headers.get('Origin') || 'localhost:5173', 'https://basedorbiased.vercel.app', 'http://localhost:5173', 'https://basedorbiased.app'];
...
export async function processHandler(request: Request, env: Env): Promise<Response> {
	...
	try {
		...
		const useTestData = !env.FAKE_API || (env.FAKE_API as unknown as string) === 'false' ? false : (env.FAKE_API as unknown as string) === 'true' || env.FAKE_API === true;
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext) {
		...
		async function handleOauthCallback(request: Request, env: Env): Promise<Response> {
			...
			const corsHeaders = {
				'Access-Control-Allow-Origin': request.headers.get('Origin') || 'https://basedorbiased.vercel.app' || 'https://basedorbiased.app' || 'http://localhost:5173',
				'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
				'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Requested-With',
				'Access-Control-Allow-Credentials': 'true',
				'Access-Control-Max-Age': '86400',
